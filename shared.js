...
const PokeFinderApp = (() => {
    ...
    const saveHistory = (pokemon) => {
        let history = getHistory().filter(p => p.id !== pokemon.id);
    ...
    const clearHistoryAndCache = () => {
        ...
        Object.keys(localStorage).forEach(key => {
            if (key.startsWith("pokemon-")) localStorage.removeItem(key);
        });
    };

    /* ================= BUSCADOR ================= */
    const buscar = async () => {
        const valor = el.input()?.value.toLowerCase().trim();
        const tipo = el.searchType()?.value || "pokemon";
        ...
        el.container().innerHTML = "⏳ Buscando...";
        ...
        el.container().innerHTML = `
            <div class="pokemon-card">
                ...
                <div style="margin-bottom:8px;">
                    <span style="padding:4px 8px;border:var(--border);
                        ...
                        <div class="stat-row">
                            ...
                            <div class="stat-bar-outer">
                                <div class="stat-bar-inner"
                                    style="width:${(s.base_stat / 150) * 100}%"></div>
                            </div>
                        </div>`).join("")}
                </div>
            </div>`;
    ...
    const renderHistory = () => {
        ...
        list.innerHTML = history.map(p => `
            <div class="pokemon-card" style="display:flex;align-items:center;gap:15px;">
                ...
                <div style="flex-grow:1;cursor:pointer;" data-id="${p.id}">
        ...
        list.onclick = (e) => {
            if (e.target.dataset.id)
                window.location.href = `index.html?pokemon=${e.target.dataset.id}`;
            if (e.target.dataset.remove) {
                removeFromHistory(Number(e.target.dataset.remove));
                renderHistory();
            }
        };
    ...
    const init = () => {
        ...
        el.searchBtn()?.addEventListener("click", buscar);
        ...
        el.container()?.addEventListener("click", (e) => {
            ...
            if (!card) return;
            el.input().value = card.dataset.name;
            buscar();
        });
        ...
        if (params.get("pokemon") && el.input()) {
            el.input().value = params.get("pokemon");
            buscar();
        }
